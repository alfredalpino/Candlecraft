#!/usr/bin/env python3
"""
Forex OHLCV Data Puller with Real-time Streaming
...
Usage:
    ...
    python pull_fx.py --symbol EUR/USD --timeframe 1h --stream
    
    # Historical + Real-time (backfill then stream)
    python pull_fx.py --symbol EUR/USD --timeframe 1h --limit 100 --stream
"""
...
def fetch_ohlcv(
    ...
    Args:
        ...
        symbol: Forex pair symbol (e.g., 'EUR/USD', 'GBP/USD')
    ...
    # Normalize symbol format (accept both EUR/USD and EUR_USD)
    ...
    try:
        ...
        if limit:
            ...
            ts = client.time_series(
                ...
                interval=interval,
                outputsize=limit,
                timezone=timezone if timezone else "Exchange",
            )
...
def stream_realtime_prices(
        ...
        try:
            ...
            if data.get("event") == "subscribe-status":
                if data.get("status") == "ok":
                    ...
                    print("  4. Try using EUR/USD or GBP/USD (more commonly available)")
    ...
    def on_open(ws):
        ...
        # Subscribe to symbol using extended format with explicit type
        # This format is more reliable for forex pairs
        subscribe_msg = {
            "action": "subscribe",
            "params": {
                "symbols": [{
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Fetch OHLCV data from Twelve Data (Forex)",
...
Examples:
  ...
  python pull_fx.py --symbol EUR/USD --timeframe 1m --limit 1 --poll
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Forex pair symbol (e.g., EUR/USD, GBP/USD, USD/JPY). Accepts both '/' and '_' separators.",
    )
    ...
    parser.add_argument(
        "--timezone",
        type=str,
        help="Timezone for timestamps. Options: 'UTC', 'Exchange' (default), or IANA timezone (e.g., 'America/New_York', 'Asia/Singapore')",
    )
